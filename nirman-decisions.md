...
**Reasoning:** Users may configure different cloud or local providers, and task roles may require different model capabilities.
...
## ADR-207: Windows host baseline evidence-based review procedure

**Status:** Proposed

**Decision:** ADR-207 establishes an evidence-based review procedure for the existing Windows host baseline. ADR-108 remains the accepted implementation decision. ADR-207 does not supersede ADR-108 or ADR-094. A future alternative may be adopted only through a new accepted ADR satisfying BS §67.7.

The review procedure requires:
1. Real Windows-runtime evidence for the existing Tauri 2 host (install, upgrade, uninstall/reinstall, startup, supervisor handshake, UI disconnect/reconnect, login startup, sleep/resume, UI crash, supervisor crash, child-process cleanup, ConPTY, notifications, ACL/security boundaries, Credential Manager/DPAPI, installer rollback, state/database preservation).
2. Evidence bound to a matching `EnvironmentCapabilityRecord` fingerprint, target platform, and source revision (per `CONTRACT.RUNTIME.PLATFORM_CAPABILITY`, BS §79).
3. Comparison evidence for any alternative candidate (WinUI 3, WPF) on the same Windows machine, same fixtures, same source revision.
4. A new accepted ADR that supersedes ADR-108 per BS §67.7 (versioned supersession through a recorded ADR, superseded contract retained and marked `DEPRECATED`) before any migration is authorized.

The review is owned by M120. M120 references `CONTRACT.RUNTIME.PLATFORM_CAPABILITY` (BS §79); it does not create a new contract.

**Rationale:** ADR-108 (Tauri 2 + React/TypeScript/Vite) is the implemented and tested host. The absence of real Windows-runtime evidence means the baseline is under-evidenced for permanent-lock status, but no evidence currently demonstrates that an alternative solves a Nirman requirement better enough to justify a second UI technology and migration boundary. The React/TS/Vite ecosystem is uniquely suited to Nirman's complex, dynamic UI requirements (chat, code editor, terminal, task graph, evidence views). The Rust control plane (ADR-109, ADR-111) is the load-bearing architecture; the UI is a presentation wrapper around it. A C#/.NET rewrite would duplicate enormous UI logic for zero architectural gain.

**Consequences:** ADR-108 holds for v1. ADR-094's scope-binding claim (Android-only target) is unaffected. The Rust control plane / supervisor split (ADR-109, ADR-111) is unaffected. The SupervisorConnection protocol (TA §57.3, §81) stays transport-agnostic. No migration is authorized until a future ADR supersedes ADR-108 with attached evaluation evidence. M120 owns the review and must produce the evidence.
