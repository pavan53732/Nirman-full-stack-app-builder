...
Implement `ScenarioSynthesizer` (TA §62.1; ADR-225): derive an `E2EScenario` for every acceptance criterion and every build spec §56.3 class from the `ScreenGraph`, register it through `ScenarioRegistry`, and write `coveredRequirementIds` and `uncoveredRequirementIds`. Fixture: a requirement no graph path can reach is reported to the planner as a `REPLAN` input and is absent from completion evidence; a synthesized scenario that flips between runs is quarantined by the TA §62.4 determinism rule exactly like an authored one.
