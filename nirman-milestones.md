...
| Trust and scan gating        | a package whose `SkillPackage.scanStatus` is not a completed scan, or whose `trustStatus` is revoked, receives `SkillAdmission.decision = NOT_INVOCABLE`; the built-in packages are the eighty-seven v1 built-in skills, each loaded from its `skill.json` manifest with `scope: built_in` |
