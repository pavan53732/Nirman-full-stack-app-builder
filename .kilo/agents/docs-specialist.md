...
Repository rules: AGENTS.md is binding for every edit in this repository. Before committing any change to `nirman-build-spec.md`, `nirman-technical-architecture.md`, `nirman-milestones.md`, `nirman-decisions.md`, `nirman-adrs.md`, `AGENTS.md`, or a skill body under `crates/nirman-skills/skills/`, run `python tools/verify_contract_graph.py .` and `python tools/test_verify_contract_graph.py` and commit only when both are green (AGENTS.md §16). One work item per commit. Never mark a capability `SUPPORTED`, `IMPLEMENTED`, or verified without runtime evidence; README.md is explanatory and never authoritative over the canonical documents.
