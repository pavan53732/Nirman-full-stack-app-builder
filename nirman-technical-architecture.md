...
Each mandatory check the plan names MUST then be resolved to exactly one skill-resolution outcome before any `SkillAdmission` is written for it, per build spec §52.10.1. The runtime resolves the implementing `SkillPackage.skillId` from the build spec §79.7 registry — never from model memory — admits it, and records the outcome in the plan's `check_bindings`, or, when the check cannot be discharged, in `unresolved_mandatory_checks`. Resolution reuses the existing `SkillAdmission.decision` value set (`ADMITTED`, `BLOCKED`, `NOT_FOUND`, `NOT_INVOCABLE`) and the existing `applicability` value `not_applicable` with its justification; it creates no new state machine, enum, or authority. A mandatory check that resolves to no record at all is a defect, and the plan MUST NOT be reported as validation-complete while any mandatory check is outstanding: the completion evaluator (build spec §5.7.7) treats an outstanding check as missing current mandatory evidence, so `NOT_EVALUATED` and `NOT_COMPLETE` remain reachable. `WorkerContract.required_skills` (§84.1) MAY mirror the same skill ids as supporting worker-admission data but is not the canonical representation; the plan's own `check_bindings` is, and a disagreement between the two is a defect. The plan and its bindings persist with the task under §57.5, survive supervisor restart, and are invalidated by a project revision, requirement revision, environment capability record, or evidence dependency change.
