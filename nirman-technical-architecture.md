...
- *Frame path.* Each frame is stamped by the supervisor with `frameSequence`, `capturedAt`, `width`, `height`, `pixelFormat`, `deviceId`, `previewRevisionId`, `artifactFingerprint`, and `deviceStateFingerprint` — the binding above — and written to a per-surface shared-memory ring (`transportKind: SHARED_MEMORY_RING`) of `ringDepth` slots (default 3); a `PreviewSyncEvent` with `eventType: OBSERVATION_CAPTURED` and `eventTruth: OBSERVED`, naming the slot and carrying the stamp, travels over the authenticated `SupervisorConnection` (§57.3). Frame pixels never travel through the durable event log; only the stamp does. The ring is the delivery mechanism, `PreviewProjectionReducer` remains the sole state authority, and a frame whose stamp fails the binding check is dropped before any event is emitted.
- *Rate and backpressure.* `maxFrameRate` is 30 frames per second for the baseline transport; when PreviewHost has not consumed a slot, the oldest unconsumed frame is overwritten (`backpressurePolicy: DROP_OLDEST`), never buffered without bound and never blocking the emulator. A gap of more than `staleAfterMs` (default 2000) without a new frame while the device reports the display unchanged is `IDLE`; the same gap with a failed heartbeat is `LOST`, which the reducer projects as disconnected under CLAUSE.PREVIEW_SYNC.NO_LOCAL_ADVANCE.
