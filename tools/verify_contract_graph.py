...
`--dump-registries` prints the parsed registries; `--emit-index` regenerates
INDEX.md (ADR-220) and exits.
...
def check_semantic_documentation(docs, R, D, root="."):
    ...
    required_contract_tokens = (
        ...
        (docs["schemas"], "SCHEMAS", "### 1.82 LockedDecision"),
        (docs["schemas"], "SCHEMAS", "- decisionProposals: List<ConversationDecision>     // conversation-local proposals; never canonical locked decisions"),
        (ta, "TA §59.1", "`MemoryRecord.class = DECISION` for retrieval, but that record is a derived semantic-memory projection"),
        (ta, "TA §86.1", "`ConversationDecision` is a proposal envelope owned by Conversation, not a canonical locked decision"),
        (docs["schemas"], "SCHEMAS", "- targetRequirementRevision: integer | null"),
        (ta, "TA §44.3.3", "Admission invalidates downstream implementation, scenario, evidence, proof, completion, preview, and artifact projections"),
        (docs["schemas"], "SCHEMAS", "### 2.130 ProviderRequestProvenance"),
        (docs["schemas"], "SCHEMAS", "- retentionDependencyRefs: list of { dependencyKind: ACTIVE_TASK | PENDING_RETRY | UNRESOLVED_EXTERNAL_EFFECT | RECOVERY | EVIDENCE | USAGE_ATTRIBUTION | AUDIT_HOLD, dependencyId }"),
        (docs["schemas"], "SCHEMAS", "- attemptState: PREPARED | ISSUED | STREAMING | COMPLETED | FAILED | CANCELLED | UNKNOWN | RECONCILING | RESOLVED"),
    ...
    for label, text in (("build spec", bs), ("architecture", ta), ("development plan", dev)):
        ...
        def _canon(p):
            words = p.split()
            while words and words[0] in ("The", "A", "An", "Each", "Every", "No", "One", "Per", "Canonical", "Its", "This", "That", "Any", "Both"):
                words = words[1:]
            return " ".join(words)
    ...
    for token in ("two levels by default",):
        for label, text in (("build spec", bs), ("architecture", ta)):
            if token in text:
                D.add("semantic documentation", "worker taxonomy", f"{label} still limits nesting to {token!r}; ADR-227 fixes three levels")
    m_227 = re.search(r"## ADR-227:.*?(?=\n## ADR-|\Z)", dec, re.S)
    m_227 = m_227.group(0) if m_227 else ""
    for needle, why in (("**Locks:** `CONTRACT.RUNTIME.AUTHORITY`", "lock CONTRACT.RUNTIME.AUTHORITY"),
                        ("**Reversal trigger:**", "carry a Reversal trigger"),
                        ("A probe child cannot create children", "bound the probe child")):
        if needle not in m_227:
            D.add("semantic documentation", "worker taxonomy", f"ADR-227 must {why}")
...
INDEX_HEADER = "# Nirman Index"
INDEX_NOTICE = ("> Generated by `python3 tools/verify_contract_graph.py --emit-index` (ADR-220). "
                "Do not edit by hand: certification fails when this file differs from the generator's output. "
                "This document is navigation only — it holds no authority, and a citation here resolves to the "
                "section it names, never to this file.")


def _heading_map(text, depth=3):
    """[(number, title)] for `## N.` / `### N.k` headings outside fences."""
    out, fence = [], False
    for line in text.split("\n"):
        ...
        if fence:
            continue
        m = re.match(r"^(#{2,%d})\s+(\d+(?:\.\d+)*)\.?\s+(.*\S)\s*$" % depth, line)
        if m:
            out.append((m.group(2), m.group(3)))
    return out


def build_index(docs, R):
    """Render INDEX.md from the parsed registries and the documents' headings.

    Sections: section map (BS, TA, SCHEMAS groups), contract -> authority
    (from §67.8), schema -> location (from the nirman-schemas.md owner lines),
    milestone -> section (from the milestone document's headings), ADR ranges
    (from nirman-adrs.md). Every row is derived; nothing here is typed in.
    """
    bs, ta = docs["bs"], docs["ta"]
    sch, dev, adrs = docs.get("schemas", ""), docs["dev"], adr_text(docs)
    ...
    if sch:
        ...
        for num, title in _heading_map(sch, depth=2):
            lines.append(f"| §{num} | {title} |")
        lines.append("")
    lines += ["## 2. Contract → authority", "", "| ContractId | Authority | Extensions | Architecture | Class |", "|---|---|---|---|---|"]
    for cid, r in sorted(R["contracts"].items()):
        lines.append(f"| `{cid}` | {r['authority']} | {r['ext']} | {r['arch']} | {r['cls']} |")
    lines.append("")
    if sch:
    ...
    ids = []
    for m in re.finditer(r"^## ADR-(\d+)([A-Z]?):(.*?)(?=^## ADR-|\Z)", adrs, re.M | re.S):
        st = re.search(r"^\*\*Status:\*\*\s*(\w+)", m.group(3), re.M)
        ids.append((int(m.group(1)), m.group(2), st.group(1) if st else "(no Status line)"))
    if ids:
        lo = ids[0][0]
        for start in range(lo - (lo % 50), ids[-1][0] + 1, 50):
            in_range = [x for x in ids if start <= x[0] < start + 50]
            if not in_range:
                continue
            statuses = {}
            for _n, _suf, st in in_range:
                statuses[st] = statuses.get(st, 0) + 1
            lines.append(f"| ADR-{in_range[0][0]:03d}{in_range[0][1]}–ADR-{in_range[-1][0]:03d}{in_range[-1][1]} | {len(in_range)} | "
                         + ", ".join(f"{k} {v}" for k, v in sorted(statuses.items())) + " |")
    lines.append("")
    return "\n".join(lines)


def check_index_drift(docs, R, D):
    """ADR-220: INDEX.md is generated; when present it must equal the
    generator's output byte for byte (reported under check 13 "structure")."""
    current = docs.get("index", "")
    if not current:
        return
    expected = build_index(docs, R)
    if current != expected:
...
def check_orchestration_hardening(docs, D):
    ...
    locks = [
        ...
        ("ta",
         ...
         "build spec §26.2: the six-stage delivery/application pipeline is gone"),
        ("schemas",
         "- processingState: RECEIVED | ACCEPTED | APPLIED | REJECTED | DEFERRED",
         "nirman-schemas.md §1.13: WorkerMessage lost the application processingState field"),
        ("ta",
         "No agent waits on an agent. Every cross-worker wait becomes a durable `AwaitCondition`",
         "technical architecture §58.11.1: the asynchronous AwaitCondition rule is gone"),
        ("schemas",
         "- quorumCount\n- joinPolicy: ALL | ANY | QUORUM(n) | OPTIONAL\n- joinRevision",
         "nirman-schemas.md §2.121: JoinBarrierState lost quorum/join-revision fields"),
        ("ta",
         "Delivery is at-least-once with idempotent authoritative application; the fabric makes no end-to-end exactly-once claim.",
         "technical architecture §57.11.2: the at-least-once + idempotent-apply guarantee is gone"),
        ("ta",
...
def main():
    ...
    emit_index = "--emit-index" in sys.argv
    args = [a for a in sys.argv[1:] if a not in ("--dump-registries", "--emit-index")]
    root = args[0] if args else "."
    if emit_index:
        docs = load(root)
        R = parse_registries(docs, Defects())
        path = os.path.join(root, DOCS["index"])
        with open(path, "w", encoding="utf-8", newline="\n") as fh:
            fh.write(build_index(docs, R))
        print(f"wrote {path}")
        return 0
    R, adj, D = verify(root)
