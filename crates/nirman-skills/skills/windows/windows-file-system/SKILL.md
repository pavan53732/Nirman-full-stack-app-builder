...
- Invariant claims this skill must leave observable, each a statement the
  ...
  - The resolved workspace root matches the configured one, not a profile default.
  - The invoking account can reach its own workspace and no other account can.
  - An interrupted write leaves no file that reads as valid.
