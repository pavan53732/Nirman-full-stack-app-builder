...
- Invariant claims this skill must leave observable, each a statement the
  ...
  - Every control is reachable by keyboard, and focus can leave every control it can enter.
  - Every status has a non-visual equivalent, not only a colour or an animation.
  - Nothing clips, overlaps, or becomes unreachable under high contrast or at a larger scale.
