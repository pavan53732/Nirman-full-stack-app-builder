...
Scope: offline-first data in Nirman apps — queueing local writes while disconnected,
detecting and resolving conflicts, replaying queued operations in a correct and
idempotent order, and reconciling local state with the server after a partition (BS §79.7).
