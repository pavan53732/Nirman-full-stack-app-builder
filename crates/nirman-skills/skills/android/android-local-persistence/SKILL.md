...
Scope: on-device persistence in a generated Android application — choosing between Room, SQLite, and DataStore, transactional writes, schema version and integrity, cache bounds and eviction, and secure storage for sensitive values (BS §79.7).
...
- Invariant claims this skill must leave observable, each a statement the
  ...
  - A committed write is readable in full after a crash, and an uncommitted one is not readable at all.
  - No sensitive value exists outside secure storage, including in backups and logs.
  - Cache size stays within its declared bound, and eviction is observable at that bound.
