...
- Invariant claims this skill must leave observable, each a statement the
  ...
  - A breaking change is released under a new version, never changed in place.
  - Every paginated collection has a deterministic ordering, so pages neither repeat nor skip records.
  - Every retryable non-safe method is idempotent.
