...
Nirman installs and builds on every 64-bit Windows 10/11 host, x86-64 or ARM64. Preflight MUST record `host_architecture` in `EnvironmentCapabilityRecord` and classify the emulator capability from it before any download is attempted. On an x86-64 host the Nirman-managed local Android emulator is provisioned per technical architecture §49.4. On a Windows ARM64 host (Snapdragon and other Arm SoCs) the Android SDK repository publishes no Windows ARM64 build of the Android Emulator, and a hypervisor-backed emulator cannot run under the x86 emulation layer; the emulator capability is therefore `UNAVAILABLE` with the stated reason `no Windows ARM64 build of the Android Emulator is published`, while the JDK, SDK, build-tools, platform-tools, and the Android build itself remain `AVAILABLE` (the ARM64 JDK is provisioned; x86-64 tools run under emulation). `ToolchainProvisioner` MUST NOT attempt to download or run an emulator or system image whose host architecture does not match, and MUST NOT present an x86-64 emulator running under the emulation layer, a container, a VM, WSL, a remote machine, or a physical device as a substitute (CLAUSE.PLATFORM.NO_SUBSTITUTE_TARGET). On such a host the split rule of §79.4 applies: synthesis, Gradle build, static analysis, host-native tests, artifact inspection, and export proceed, the Preview panel shows the truthful unavailable state and its reason rather than a waiting indicator, and the completion evaluator reports at most `SUPPORTED_WITH_ENVIRONMENT_REQUIREMENTS` (CLAUSE.PLATFORM.NO_RUNTIME_INFERENCE), never `SUPPORTED`. The classification is host-derived and deterministic (CLAUSE.PLATFORM.DETERMINISTIC_CLASSIFICATION): when the SDK repository publishes a Windows ARM64 emulator, a new `ToolchainProvisioningManifest` version adds the package and the ARM64 host becomes an x86-64-equivalent host with no other change (ADR-221 reversal trigger).
