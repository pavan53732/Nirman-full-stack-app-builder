...
| BS §4.2 | "The setup wizard should check the local environment, detect installed versions of Node.js, package managers, Java, Gradle, Android SDK, platform-tools, emulator tooling, and identify which Android capabilities are available." | MUST check the local environment during setup | Setup detects and reports each listed toolchain version and derives the resulting Android capability set from them |
