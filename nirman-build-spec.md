...
`InterfaceAgreement` is a pre-dispatch contract, not descriptive metadata. Before any write-capable worker is launched, the runtime MUST run `INTERFACE_COMPLETE`. The selected work shape determines the required agreement fields; every required field MUST be populated, or the field MUST carry an explicit `INAPPLICABLE` reason. `INTERFACE_COMPLETE` MUST also verify `taskGraphRevision`, `planRevision`, `projectRevision`, `parentTaskId`, `contextIntegrityHash`, and the affected task's `taskRevisionId`. Failure rejects dispatch and returns the work item to planning; it does not create a partially authorized worker. Before dispatching any task to a specialized worker, the runtime MUST also pass the four-dimensional `WorkerCompatibilityValidator` audit (technical architecture §71.8), asserting context-capacity headroom, multimodal vision requirements, schema-handling fidelity, and sandbox isolation profile compatibility. During worker execution, the supervisor's `WorkerAnomalyDetector` (technical architecture §58.1) continuously audits progress against cognitive stall, mutation thrashing, and schema deviation thresholds, revoking leases and quarantining offending assignments upon violation. Before staging Android code mutations, `AndroidDataFlowAnalyzer` (technical architecture §47.4) validates intra-procedural coroutine lifecycle binding and verifies sensitive data-flow taint paths to protect against memory leaks and credential exposure; `AndroidAntiPatternDetector` (technical architecture §47.4) rejects prohibited Android constructs and Compose allocation anti-patterns; and `AndroidCircularDependencyDetector` (technical architecture §47.4) verifies that module, injection, and database entity dependency graphs remain acyclic; `ArchitectureDriftDetector` (technical architecture §47.4) flags architectural boundary violations between UI, ViewModel, and persistence layers; and `CodeDuplicationDetector` (technical architecture §47.4) identifies duplicated Composable UI subtrees and business logic algorithms to suggest refactoring into reusable components. Fan-in across workers MUST durably record join state (`JoinBarrierState`, §80.5.4): wake conditions are never inferred from transport traffic.
...
Nirman should implement a three-outcome policy engine: **allow**, **ask**, and **deny**. Policies should be evaluated against the tool, command, path, project, worker role, network destination, and action risk class; there is no operating-mode dimension because there is one mode (§23.3; ADR-226). The implementation of this engine is `PolicyAuthority` (technical architecture §44.1, §57.12). Within `PolicyAuthority`, the `ApprovalGatePolicyEngine` sub-module produces the three-outcome decision, `RiskClassifier` assigns the action risk class (`ROUTINE`, `REVIEWABLE`, `PRIVILEGED`, or `HARD_GATED`) before each decision, and `DestructiveActionConfirmationFlow` is the `HARD_GATED` approval path for destructive operations. `MidRunEditCoordinator` (technical architecture §44.3.1) routes mid-session manual file edits through `PolicyAuthority` and `ConstructionTransactionManager` without interrupting the autonomous loop. Autonomy levels (§28.5) are observed evidence states reported by the runtime — they are never user-selectable modes, and `PolicyAuthority` never exposes an autonomy dial or a step-through mode (ADR-226).
...
A repeated-action guard should detect when the same tool call, command, or failed repair is repeated without progress. A detected repetition is a stall in the sense of §29.4: it is handed to `RecoveryAuthority`, which ascends the §28.2 ladder, and it is explained in the task record as a “possible loop” finding with the fingerprint and the rung selected. The guard never pauses the task and never waits for the user (ADR-226); `task.pause` is a user command that no runtime component issues. A detected repetition is a stall: RecoveryAuthority MUST select the next authorized recovery action from a materially different strategy class — evidence acquisition, strategy change, delegation, backtracking, branching, escalation, or truthful termination. The same materially equivalent action MUST NOT be retried.
...
Notification is a presentation event and has no execution authority. A notification MUST NOT transition a task into USER_REQUIRED. A notification MUST NOT pause autonomous execution.
...
**Android runtime sub-ladder (ADR-225).** Below the strategy-change rungs of the ladder sits an ordered runtime sub-ladder that `RecoveryAuthority` selects deterministically from the failure family of the fingerprint — never the model: (a) reload the running application (Compose reload, fast refresh, or incremental install per §44.1); (b) reseed — restore the `GoldenSnapshot`, reinstall, and re-apply `seedData`; (c) force-stop and relaunch the application; (d) uninstall and reinstall the artifact; (e) release and re-boot the emulator session; (f) re-verify and, if needed, re-provision the toolchain through `ToolchainProvisioner`. An install, launch, ANR, emulator, or ADB failure family enters at the lowest rung its fingerprint has not yet exhausted; a compile, resource, dependency, or signing family skips the sub-ladder entirely, because no runtime action can repair a build. For source compilation, resource linkage, and static Android API level failures, the `Debugging Worker` queries `EpisodicRepairPatternCatalog` (technical architecture §47.4) for proven, metamorphic-verified AST repair transformations matching the failure signature before triggering model-based deliberation passes. For runtime crashes and fatal exceptions, `CrashPatternAnalyzer` (technical architecture §47.4) parses Logcat stack traces, correlates failure frames with source symbols in `AndroidSymbolGraph`, and queries `EpisodicRepairPatternCatalog` for targeted repair transformations before model deliberation. Each rung is one recovery attempt with its own fingerprint entry; a rung that fails to change the observation is exhausted for that fingerprint and is not retried before the ladder moves up. Only when the sub-ladder is exhausted does the failure reach strategy change, checkpoint restoration, and the higher rungs.
...
`WorkerHandoff` is admissible only when its project revision, task-graph revision, plan revision, context integrity, lease/attempt, reservation epoch, source fingerprints, and evidence watermark still match authoritative state. Otherwise reconciliation rejects the handoff as stale and requests re-grounding/revalidation. When a worker process fails, times out, or is evicted, the supervisor executes `WorkerFailoverReconstitutionProtocol` (technical architecture §58.1), reseeding clean context from the last validated checkpoint and quarantining tainted intermediate edits before dispatching a replacement worker.
...
`CompletionReportComposer` (technical architecture §44.3.2, §57.12) assembles the final `TaskResult` (§23.9, SCHEMAS §1.11) from the `EvidenceAuthority` ledger, `EventStore` records, and `ArtifactAuthority` promotion records when `EvidenceAuthority` issues the completion decision. It reads authoritative records only — it does not produce evidence or decide completion. `OutputWalkthroughGenerator` (technical architecture §44.3.2, §57.12) produces the accompanying human-readable post-task walkthrough artifact from the `EpisodeRecord` and committed event sequence, persisted in the task directory and linked from the completion screen. Both are read-only composition modules with no authority.
...
`ConstraintRegistry` is also the sole admission and persistence authority for canonical `LockedDecision` records. A decision retains `decisionId` while wording, rationale, source lineage, or scope is revised; each accepted change increments `decisionRevision`. A semantic replacement that must coexist historically receives a new identity linked by `supersedes`/`supersededBy`. `MemoryRecord.class = DECISION` is retrieval-oriented semantic memory and cannot admit, unlock, supersede, or replace a `LockedDecision`.
...
The typed query interface over this code-intelligence graph — symbol lookup, cross-reference expansion, fingerprint retrieval, impact-scope expansion, repair-pattern lookup, and analysis scheduling — is exposed to the agent kernel and registered IPC command handlers by `AndroidCodeIntelligenceService` (TA §47.5.1). `AndroidCodeIntelligenceService` is a read-only facade; it creates no second authority and routes all mutation proposals through the `MutationBroker` governed by §43.2. Pre-transaction architectural what-if analysis is provided by `AndroidArchitectureReasoningService` (TA §47.5.2), which traverses the `AndroidSymbolGraph` and `ImpactGraph` to compute a hypothetical impact surface before any `ConstructionTransaction` opens. On-demand code generation patterns, placeholder detection, syntactic truncation validation, and mock double leakage prevention are coordinated by `AndroidGenerationIntelligenceService` (TA §47.5.3). Staged mutations are scanned for unexpanded `TODO`, `FIXME`, and `Lorem ipsum` residue by `PlaceholderResidueDetector` (TA §47.4), premature stream cut-offs and delimiter imbalances are caught by `TruncatedFileDetector` (TA §47.4), and unauthorized mock doubles in production source sets are blocked by `MockResidualDetector` (TA §47.4) prior to transaction commit. Data layer schema validation, Room migration verification, query performance inspection, and offline-first synchronization planning are coordinated by `AndroidDataIntelligenceService` (TA §47.5.4). Staged Room database mutations are verified for migration safety and schema diff integrity by `RoomSchemaMigrationAnalyzer` (TA §47.4), Room DAO queries are inspected for N+1 patterns and SQL injection safety by `QueryPerformanceAnalyzer` (TA §47.4), and offline-first repositories and WorkManager outbox retry queues are validated by `OfflineSyncProtocolPlanner` (TA §47.4). API contract drift detection, third-party vendor integration verification, and mobile authentication security hardening are coordinated by `AndroidIntegrationIntelligenceService` (TA §74.7.1). Staged API models and client interfaces are audited for breaking changes and contract drift by `ApiContractDriftDetector` (TA §74.7.2), third-party SDK wrappers and webhook HMAC signatures are verified by `ThirdPartyIntegrationAnalyzer` (TA §74.7.3), and mobile OAuth PKCE, token refresh mutexes, and Keystore encryption are enforced by `AuthFlowSecurityHardener` (TA §74.7.4). Design token adherence, visual layout hierarchy, mobile accessibility compliance, string externalization, and dark pattern prevention are coordinated by `AndroidDesignIntelligenceService` (TA §73.16.1). Rendered frames and Compose layout trees are evaluated for contrast floors, layout overflows, and density bucket coverage by `VisualHierarchyAnalyzer` (TA §73.16.2), Android accessibility touch targets, TalkBack semantics, and focus order are audited by `AndroidAccessibilityAuditor` (TA §73.16.3), hardcoded UI strings, RTL layout mirroring, and localized plurals are validated by `StringExternalizationEngine` (TA §73.16.4), and deceptive UI hierarchies and pre-checked consent boxes are flagged by `DarkPatternDetector` (TA §73.16.5). Personal data privacy protection, PII field classification, data minimization compliance, privacy documentation, and open-source license attribution are coordinated by `AndroidPrivacyIntelligenceService` (TA §70.7.1). Sensitive data fields across Room entities, Compose inputs, and DTOs are classified by `PiiFieldClassifier` (TA §70.7.2), personal data collection is audited against functional necessity to prevent over-collection by `DataMinimizationChecker` (TA §70.7.3), legally grounded privacy policies and Google Play Data Safety forms are synthesized by `PrivacyPolicyGenerator` (TA §70.7.4), and third-party library licenses from verified SBOM records are composed into in-app notices by `OpenSourceNoticeComposer` (TA §70.7.5). Generated application observability scaffolding, in-app crash reporting, performance metrics, system tracing, debug diagnostics screens, and analytics event schemas are coordinated by `AndroidAppObservabilityService` (TA §73.17.1). Structured Logcat wrappers and R8 log-stripping rules are synthesized by `StructuredLoggingScaffolder` (TA §73.17.2), client-side uncaught exception hooks and local crash persistence are integrated by `CrashReportingScaffolder` (TA §73.17.3), AndroidX JankStats and startup latency markers are instrumented by `AppMetricsScaffolder` (TA §73.17.4), Perfetto trace sections are scaffolded by `TracingInstrumentationScaffolder` (TA §73.17.5), debug-variant health dashboards and diagnostic ZIP exporters are synthesized by `InAppDiagnosticsScaffolder` (TA §73.17.6), type-safe analytics event taxonomies are generated by `AnalyticsSchemaGenerator` (TA §73.17.7), and local feature adoption tracking is scaffolded by `FeatureUsageTracker` (TA §73.17.8). Android OS platform-specific generation, manifest permissions, Gradle configuration, shrinker rules, system notifications, deep links, and target SDK compliance are coordinated by `AndroidPlatformTargetService` (TA §73.18.1). Manifest `<uses-permission>` tags and runtime permission flows are derived by `ManifestPermissionDeriver` (TA §73.18.2), Kotlin DSL build scripts and version catalogs are reconciled by `GradleConfigSynthesizer` (TA §73.18.3), ProGuard/R8 consumer rules are synthesized by `ShrinkerRuleGenerator` (TA §73.18.4), Android 8.0+ notification channels and Android 13+ permission flows are created by `NotificationChannelSetup` (TA §73.18.5), manifest intent-filters and Navigation Compose deep links are generated by `DeepLinkIntentFilterGenerator` (TA §73.18.6), and Google Play target SDK submission deadlines are tracked by `TargetApiDeadlineTracker` (TA §73.18.7).

### Intelligence result boundary

Android intelligence components are analytical producers, never authorities. Their outputs MUST be typed, project/revision-bound, freshness-bound, and evidence-referenced. Only an existing canonical authority may interpret an intelligence result as a validation or promotion input. An analyzer/service cannot independently approve, deny, block, mutate, promote, or complete work. When no canonical result schema exists, the implementation MUST extend the owning finding/observation/evidence schema rather than create an opaque intelligence-specific authority or completion record (ADR-228).
...
**Frontier-first action selection (ADR-225).** `SELECT_ACTION` is bound to the `EvidenceFrontier` (§53.6), not to the model's preference. When the requirement the cycle is working on has an `UNRESOLVED`, `CONTRADICTED`, or `REQUIRED_VALIDATION` frontier item, or an untested discriminating test for an open hypothesis (§66.6), the kernel MUST select an observation action — a scenario run, a state probe, a UI-hierarchy or Logcat capture, a focused test — before any mutation action for that requirement. A mutation proposal MUST name the frontier item it targets (`targetFrontierItemId`) and the observation that motivated it (`motivatingEvidenceId`); a mutation proposal that names neither is rejected at `AUTHORIZE` as `EVIDENCE_NOT_ACQUIRED`, and the rejection is a fingerprinted event so that a worker repeatedly editing without observing surfaces in the stall detector (§29.4) rather than in the token count. The rule is deterministic runtime behavior: the model may propose either kind of action, but only the kernel decides which kind is admissible in the current frontier state.
...
A model statement is never a memory write. Only validated events, approved decisions, and user confirmations produce memory records, consistent with §37 (Event, Evidence, and Completion Authority). Structural code artifacts and AST repair templates are indexed using normalized structural hashes produced by `SemanticCodeFingerprintEngine` (technical architecture §47.4) to eliminate no-op edits and preserve cross-session repair lineage.
...
Every declared dependency must be resolved to an exact version with an integrity hash. The runtime must reject a dependency that cannot be resolved reproducibly, that resolves to a different artifact than previously recorded, or whose name closely resembles a known package in a way consistent with substitution attacks. `DependencyIntelligenceService` (TA §53.8.1) is the typed query coordination facade that provides kernel workers and IPC command handlers with unified access to dependency verification status, substitution detection results, SBOM completeness, and finding dispositions.
...
| CONTRACT.RUNTIME.AGENT_BUILDABILITY | BS §80 | — | N/A (INTERNAL predicate) | ADR-231, ADR-234, ADR-243, ADR-248, ADR-249, ADR-250, ADR-253, ADR-255 | M93 | INTERNAL |
...
| CONTRACT.RUNTIME.CONTEXT_GOVERNANCE | BS §74 | — | TA §79 | ADR-199, ADR-219, ADR-254, ADR-256 | M113 | CROSS_CUTTING |
...
| CONTRACT.RUNTIME.BACKGROUND_CONTINUITY | BS §77 | — | TA §82 | ADR-202, ADR-256 | M116 | CROSS_CUTTING |
...
**Negative mutation fixture:** The verifier must detect and report as a defect any corpus passage that introduces an authoritative Nirman-owned AI token, request, monetary, reasoning, pass, or autonomous-duration budget as an execution control, including any semantic alias listed in the NO_AI_USAGE_AUTHORITY check (AI quota, inference cost, model credit, reasoning credit, prompt cost, completion token, embedding cost, throughput token, context credit, token pool, request budget, cost cap, model allowance, reasoning allowance, pass allowance, duration allowance, autonomous budget, execution credit, or any field whose semantics are AI-usage-quantity-with-authority). The verifier must specifically reject fields whose names avoid the canonical vocabulary (e.g., "inference_quota", "model_credit", "reasoning_allowance", "prompt_budget") by checking that no field in any canonical schema, parameter table, or contract has execution-authority semantics while measuring AI-usage-quantity. Usage telemetry fields are explicitly excluded from this rule.
...
| CONTRACT.RUNTIME.AUTHORITY | CAP.ANDROID.GENERATE | BS §33 | BS §33 | TA §21 | TA §27.1 | BS §33 | TA §23.1 | TA §28 | ADR-066, ADR-253, ADR-254, ADR-255, ADR-256 | M65 | TEST-GEN-001 | EV-GEN-001 |
| CONTRACT.RUNTIME.EVIDENCE | CAP.ANDROID.GENERATE | BS §37 | BS §37 | TA §23 | TA §23.3 | BS §37 | TA §23.3 | TA §28 | ADR-071, ADR-254, ADR-256 | M65 | TEST-GEN-001 | EV-GEN-001 |
| CONTRACT.RUNTIME.MEMORY | CAP.ANDROID.LONG_HORIZON | BS §38 | BS §38 | TA §59 | TA §59.2 | BS §38 | TA §59.5 | TA §59.6 | ADR-140, ADR-255 | M81 | TEST-MEM-001 | EV-MEM-001 |
...
| CONTRACT.RUNTIME.CONTEXT_GOVERNANCE | CAP.ANDROID.CONTEXT_GOVERNANCE | BS §74 | BS §74 | TA §79 | TA §79.1 | BS §74 | TA §79.2 | TA §79.3 | ADR-199, ADR-219, ADR-254, ADR-256 | M113 | TEST-CONTEXT-001 | EV-CONTEXT-001 |
...
| CONTRACT.RUNTIME.BACKGROUND_CONTINUITY | CAP.ANDROID.BACKGROUND_CONTINUITY | BS §77 | BS §77 | TA §82 | TA §82.1 | BS §77 | TA §82.2 | TA §82.3 | ADR-202, ADR-256 | M116 | TEST-BG-001 | EV-BG-001 |
...
| CONTRACT.RUNTIME.AGENT_BUILDABILITY | CAP.ANDROID.CERTIFIED_RELEASE | BS §80 | BS §80 | N/A (INTERNAL predicate) | N/A (INTERNAL predicate) | BS §80 | N/A (INTERNAL predicate) | BS §80 | ADR-231, ADR-234, ADR-243, ADR-248, ADR-249, ADR-250, ADR-253, ADR-255 | M93 | TEST-INV-001 | EV-INV-001 |
...
| CONTRACT.RUNTIME.CONVERSATION_CONTEXT | CAP.ANDROID.CONVERSATION_CONTEXT | BS §82 | BS §82 | TA §86 | TA §86.1 | BS §82 | TA §86.2 | TA §86.3 | ADR-212, ADR-253, ADR-255 | M121 | TEST-CONV-001 | EV-CONV-001 |
...
The critic produces findings and evidence requests only. It has no mutation capability, no authority to approve, and no capability to mark work complete. Critique is mandatory for architectural decisions, destructive migrations, security-relevant changes, concurrency changes, state-machine changes, authentication changes, data migrations, and release packaging. As formalized in the `StrategyCritic` Pre-Implementation Counterfactual Fault Audit (technical architecture §72.7), the critique must explicitly evaluate five operational hazards before strategy authorization: (1) schema regression, (2) lifecycle mismatch, (3) concurrency race hazards, (4) offline-first divergence, and (5) dependency breaking changes. The presence of any unmitigated operational hazard blocks strategy authorization and forces causal strategy revision.
...
An inapplicable stage MUST be represented by `applicability: not_applicable` and a reason. A read-only in-process helper need not manufacture an external-effect record, but no boundary may use inapplicability to avoid a required permission, authority, evidence, compatibility, or invalidation link. A boundary envelope references `IntegrationOperationality`, `ExternalEffectRecord`, `PreviewRevision`, `OperationCapability`, `ProviderContextEnvelope`, `ArtifactSet`, `SigningIdentityBinding`, and `DocumentationCertificationReport` when those specialized contracts apply. It does not create a second lifecycle, transaction, evidence, preview, provider, skill, artifact, signing, or completion authority. A boundary whose destination is a supporting Android service integration is satisfied only when the referenced `IntegrationOperationality.aggregateState` meets the integration's declared `requiredOperationality` (§5.7.5; technical architecture §74.1); a boundary fixture that passes with `aggregateState` below `requiredOperationality` is vacuous and MUST be rejected. For external integrations crossing integration boundaries, boundary validation requires deterministic coverage of applicable fault modes via `ContractDoubleScenario` (technical architecture §74.1, SCHEMAS §2.132; ADR-258) before mock-assisted verification is accepted. Evidence produced from such doubles carries the `DOUBLE_BACKED` evidence label and cannot promote a real integration to `FUNCTIONAL`.
...
Resource optimization MAY use historical execution measurements to improve scheduling, concurrency, recovery ordering, and validation reservation. Such optimization MUST NOT become an AI-usage, provider-request, monetary, reasoning, or autonomous-duration budget.
...
### 76.7 Generated-app observability and telemetry boundaries

Generated application observability distinguishes local development instrumentation from production telemetry:
1. *Local debug and validation instrumentation:* Logcat logging, startup latency markers, and validation instrumentation may be generated as needed for development and emulator verification.
2. *Production telemetry:* Production crash reporting, remote analytics, or user tracking require explicit user intent in the product contract, a declared destination backend, explicit end-user consent mechanisms, clear retention policies, and data minimization.
3. *Nirman ingestion boundary:* Nirman itself never ingests published-app telemetry or user data.
4. *Release hygiene:* Validation-only and debug instrumentation are stripped from release artifacts via build-variant source isolation and R8 rules, proven by release verification fixtures.
...
| `android-toolchain` | Java, Gradle, Android SDK, platform tools, emulator, native dependencies, signing | Android toolchain authority (TA §49); independent of host-target build capability |
...
| `android-device-state` | observing and reconciling the state of the Nirman-managed emulator and the application on it — device and application fingerprints, install and session state, orientation and configuration, connectivity, and power state (BS §79.7). | `ANDROID_BUILD_TOOLCHAIN`, `ANDROID_EMULATOR_EXECUTION`, `ANDROID_UI_OBSERVATION` |
| `android-permission-runtime` | the runtime permission model — declaring a permission versus requesting it, permission groups, one-time and partial grants, rationale and the don't-ask-again path, special permissions outside the normal dialog, and verifying the permission state a flow actually produced (BS §79.7). | `ANDROID_BUILD_TOOLCHAIN`, `ANDROID_EMULATOR_EXECUTION`, `ANDROID_UI_OBSERVATION`, `ANDROID_INTERACTION_EXECUTION` |
| `android-lifecycle-expert` | component and process lifecycle — configuration change and recreation, state save and restore, process death and restoration, lifecycle-aware collection, foreground and background transitions, and the regression cases a lifecycle must survive (BS §79.7). | `ANDROID_BUILD_TOOLCHAIN`, `ANDROID_SOURCE_ENGINEERING` |
| `android-process-memory-expert` | process and memory health on the Nirman-managed emulator — heap growth and leak detection, out-of-memory classification, process importance and the low-memory killer, background execution limits, and memory regression measurement (BS §79.7). | `ANDROID_BUILD_TOOLCHAIN`, `ANDROID_EMULATOR_EXECUTION`, `ANDROID_PERFORMANCE_VALIDATION` |
...
| `android-contract-double-authoring` | authoring the fixtures and schema-derived bodies a supervisor-owned `ContractDouble` serves for a declared integration — declared scenario coverage, request, response, and error bodies generated from the integration's declared schema references, deterministic fixture values, and `DOUBLE_BACKED` evidence classification; the double is supervisor-owned and loopback-bound, so this skill never opens it, never reaches it, and never contacts the real service (BS §79.7; TA §74.1; BS §76.5). | `HOST_TOOL_OBSERVATION` |
| `android-content-authoring` | product copy, localization strings, and accessibility text authored as `ContentMutation` proposals carrying a `ContentRevisionDraft` — terminology profile conformance, accessibility text that describes purpose rather than restating a label, placeholder name and order identity across locales, and declared dependency propagation scope; the worker proposes only, and admission, validation, revision minting, and persistence belong to the content authorities (BS §79.7; TA §85.1). | `HOST_TOOL_OBSERVATION` |
| `android-documentation` | a managed project's documentation, decision records, and release notes — binding every document to the revision it describes so it can be invalidated, recording a decision with its alternatives, consequences, and reversal condition, keeping status claims inside the real status vocabulary, and citing evidence rather than asserting it; the handbook and artifact release report are generated by their owning services and are never hand-authored over (BS §79.7; TA §53.9). | `HOST_TOOL_OBSERVATION` |
...
| `android-local-persistence` | on-device persistence in a generated Android application — choosing between Room, SQLite, and DataStore, transactional writes, schema version and integrity, cache bounds and eviction, and secure storage for sensitive values | `HOST_TOOL_OBSERVATION` |
| `android-data-migration` | schema and data migrations for a generated Android application's on-device database — chain ordering and versioning, backward compatibility, reversible versus destructive steps, and verifying integrity after a migration | `HOST_TOOL_OBSERVATION` |
| `android-authentication-flows` | authentication and session flows in a generated Android application — credential handling, token acquisition and refresh, session expiry and revocation, and the failure states a user actually sees | `HOST_TOOL_OBSERVATION` |
| `android-payment-integration` | payment flows in a generated Android application — Play Billing capability confirmation, purchase acknowledgement that is idempotent, real-time developer notification verification and de-duplication, and reconciling provider state against local records | `HOST_TOOL_OBSERVATION` |
| `android-push-notifications` | push notification delivery for a generated Android application — the Firebase Cloud Messaging token lifecycle and its refresh, delivery treated as best-effort, payload limits, de-duplication, and the permission states a user actually sees | `HOST_TOOL_OBSERVATION` |
| `android-ui-design-system` | design system consistency across a generated Android application's surfaces — tokens over literals, component reuse, spacing and typography on the declared scale, and theme and dark mode correctness | `HOST_TOOL_OBSERVATION` |
...
| `android-independent-review` | independent adversarial review of a generated Android project — attack the construction plan, validation strategy, and completion claim before authorization and promotion, emitting read-only blocking, warning, and informational findings that name the evidence that would refute or confirm each claim | `ANDROID_SOURCE_ENGINEERING`, `ANDROID_EMULATOR_EXECUTION`, `ANDROID_RELEASE_VALIDATION` |
| `android-release-readiness` | the cross-cutting release gate for a generated Android project — aggregate the per-domain evidence into one readiness verdict with per-domain outcomes and explicit BLOCKED reasons; reports readiness, never promotes | `ANDROID_RELEASE_VALIDATION`, `ANDROID_ARTIFACT_INSPECTION`, `ANDROID_SIGNING_INSPECTION`, `ANDROID_PACKAGING` |
| `android-security-audit` | audit of the finished Android artifact and its source — secrets, exported components, cleartext policy, debuggable release flags, network-security posture, and keystore usage, with an aggregate audit of per-finding severity and evidence | `ANDROID_SOURCE_ENGINEERING`, `ANDROID_AUTHENTICATION`, `ANDROID_SIGNING_INSPECTION` |
...
| `android-contract-double-authoring` | `HOST_TOOL_OBSERVATION` |
| `android-content-authoring` | `HOST_TOOL_OBSERVATION` |
| `android-documentation` | `HOST_TOOL_OBSERVATION` |
...
| `android-independent-review` | `ANDROID_SOURCE_ENGINEERING`, `ANDROID_EMULATOR_EXECUTION`, `ANDROID_RELEASE_VALIDATION` |
| `android-release-readiness` | `ANDROID_RELEASE_VALIDATION`, `ANDROID_ARTIFACT_INSPECTION`, `ANDROID_SIGNING_INSPECTION`, `ANDROID_PACKAGING` |
| `android-security-audit` | `ANDROID_SOURCE_ENGINEERING`, `ANDROID_AUTHENTICATION`, `ANDROID_SIGNING_INSPECTION` |
...
Workspace roots MUST be allocated under a short deterministic prefix (pattern `C:\Nirman\<sid8>\p\<8-char-id>\`, where `<sid8>` is the first eight lowercase hex characters of the SHA-256 of the invoking Windows account's SID, so two accounts never share a root and no username enters a path), never under the user profile, Desktop, or a OneDrive-synced path. `C:\Nirman\<sid8>\` is the single per-user root for workspaces (`p\`) and the provisioned toolchain (`tc\`, technical architecture §49.4); the supervisor creates it on first launch with an ACL granting the invoking account and no other interactive user. `EnvironmentCapabilityPlanner` MUST probe effective maximum path length at preflight and classify `AVAILABLE` / `REPAIRABLE` / `USER_REQUIRED` / `UNAVAILABLE` per CLAUSE.PLATFORM.DETERMINISTIC_CLASSIFICATION. Enabling the long-paths registry policy requires elevation and a reboot; it is `USER_REQUIRED` and MUST NOT be performed silently. Path-length exhaustion MUST classify as a distinct failure with a stated remedy, not a generic build failure. Route it through the existing `AndroidRepairRegistry` (ADR-075). The probe result MUST be part of `environment_fingerprint`, so a change invalidates prior evidence per CLAUSE.PLATFORM.EVIDENCE_ENV_BINDING.
...
### 79.17 Host CPU architecture

Nirman's current host contract is Windows 10/11 x64 (ADR-257). Preflight MUST record `host_architecture` in `EnvironmentCapabilityRecord` and accept only `hostArchitecture = X64`. Windows ARM64 is OUT_OF_SCOPE and installation or startup must stop before toolchain provisioning with a truthful unsupported host result (`HOST_OUT_OF_SCOPE`). Nirman MUST NOT claim build-only, partial, emulated, remote, containerized, WSL, physical-device, or reduced-capability ARM64 support (CLAUSE.PLATFORM.NO_SUBSTITUTE_TARGET). `ToolchainProvisioner` MUST NOT download or provision an ARM64 JDK, SDK, or tools on an ARM64 host.

Guest ABI is a separate concern from host architecture. The provisioned system image is x86_64; an application dependency that ships only arm64-v8a native libraries runs on it through the image's Arm translation layer, slowly, or not at all. The technology resolver (technical architecture §10.5) MUST prefer dependencies that ship x86_64 native libraries, MUST record any arm64-only native dependency in the `AndroidTechnologyPlan` as an environment requirement of the emulator profile, and MUST classify a validation that such a dependency prevents as `UNAVAILABLE` with that reason rather than as an application failure.
...
| BS §3.4 | "The user should be able to undo the complete task, restore an individual file, compare versions, or continue from a previous checkpoint." | MUST offer undo, single-file restore, compare, and resume | All four recovery affordances are available on a finished task |
| BS §3.5 | "Nirman should start with a reliable dynamic project-synthesis loop" | MUST start with synthesis | First vertical slice MUST be synthesis loop |
| BS §4.1 | "Nirman should use a minimal, focused desktop layout inspired by modern AI coding products without copying their branding or exact interface" | MUST use minimal layout | Layout MUST have: left nav, chat, file tree, workspace, bottom panel, toolbar |
| BS §4.2 | "On first launch, Nirman should explain that it is a local desktop application and ask the user to choose an AI provider" | MUST explain on first launch | First-run screen MUST explain local-first nature |
| BS §4.2 | "The setup wizard should check the local environment, detect the installed Java, Gradle, Android SDK, platform-tools, and emulator tooling versions, and identify which Android capabilities are available." | MUST check the local environment during setup | Setup detects and reports each listed Android toolchain version and derives the resulting Android capability set from them |
...
| BS §4.4 | "Nirman should optionally capture screenshots during autonomous tasks and compare them with user-provided references or generated visual baselines." | MAY capture screenshots; MUST compare when captured | Capture is opt-in; a comparison requires a user reference or a generated baseline |
| BS §4.4 | "The user should be told when an image is being sent to a cloud provider." | MUST notify before an image leaves for a cloud provider | The notice precedes transmission and is not suppressible |
| BS §4.5 | "The application should include a full code editor with syntax highlighting, search, multi-file tabs, formatting, diagnostics, and direct editing" | MUST include editor | Editor MUST have: syntax highlighting, search, tabs, formatting, diagnostics |
| BS §4.5 | "After a manual edit, the agent should be able to re-index the project and continue working from the updated state." | MUST re-index after a manual edit | Re-indexing restores continuation from the updated project state |
| BS §5.1 | "should focus exclusively on Android" | MUST focus on Android | Generated target is Android only |
| BS §5.1 | "The generator should synthesize the project structure, screens, navigation, state model, data layer, permissions, services, assets, design system, tests, build configuration, and validation plan for each request." | MUST synthesize all twelve listed artifacts | Every request produces the full listed artifact set; a missing artifact is an incomplete synthesis |
| BS §5.1 | "The user should be able to say what the application must do without first knowing which framework or template to select." | MUST accept intent with no framework choice | Framework and template resolution belongs to the §5.1 resolver, never to the user |
| BS §5.1 | "It should build each application from the user’s natural-language requirements, reference screenshots, supplied assets, existing project files, device assumptions, and requested integrations." | MUST build from the full declared input set | Natural-language requirements, reference screenshots, supplied assets, existing files, device assumptions, and requested integrations are all inputs |
| BS §5.2 | "Nirman must be designed to build all categories of Android applications end to end" | MUST build all categories | Technology resolver MUST support all listed categories |
...
| BS §6.5 | "Nirman should not send the entire project to the model on every request" | MUST use index | Context selector MUST use project index |
| BS §6.5 | "The project index should be refreshed after manual edits, generated changes, dependency installation, and branch or checkpoint changes." | MUST refresh the index on all four triggers | A stale index after any listed event is a defect |
| BS §6.5 | "It should maintain a lightweight index of files, symbols, routes, components, configuration, scripts, and recent changes." | MUST maintain the project index | Files, symbols, routes, components, configuration, scripts, and recent changes are indexed |
| BS §6.5 | "The context selector should retrieve only the files and summaries relevant to the current task." | MUST retrieve only task-relevant context | Retrieval beyond the current task's relevance is a defect |
| BS §6.6 | "Before an autonomous task changes multiple files, Nirman should create a checkpoint" | MUST create checkpoint | Before multi-file changes |
| BS §6.6 | "The user should be able to undo the whole task, restore a previous checkpoint, or inspect a file-by-file diff." | MUST offer undo, checkpoint restore, and file diff | All three recovery affordances are available on a finished task |
| BS §6.6 | "Git should be used where available, but Nirman should still provide understandable recovery behavior for projects that are not yet Git repositories." | MUST use Git when present and still recover without it | A non-Git project retains checkpoint-based recovery |
| BS §6.6 | "A checkpoint should record the project revision, task description, generated plan, provider identity, and changed-file set." | MUST record all five checkpoint attributes | Project revision, task description, generated plan, provider identity, and changed-file set |
| BS §7 | "The AI model should interact with Nirman through structured tools" | MUST use structured tools | Model MUST use only defined tools |
| BS §7.3 | "The agent should classify failures instead of blindly retrying" | MUST classify | Agent MUST classify every failure |
| BS §7.3 | "should stop and explain" | MUST stop and explain | After repeated failure, MUST stop and explain |
| BS §7.3 | "It should include the relevant command, error output, suspected cause, attempted fixes, and the specific user action required." | MUST include all five diagnostic elements | Command, error output, suspected cause, attempted fixes, and the required user action |
| BS §8.1 | "should allow users to configure" | MUST allow configuration | Users MUST be able to configure all listed fields |
| BS §8.2 | "A provider adapter should return a normalized result containing the model ID, response text, tool calls, structured output, reasoning usage when available, capability metadata, usage information, finish reason, request duration, and any provider warning" | MUST return normalized result | Adapter MUST return normalized result |
| BS §8.3 | "Nirman must clearly communicate whether project content is being sent to a cloud model" | MUST communicate | MUST tell user when content is sent to cloud |
| BS §8.3 | "The user should be able to configure context policies that exclude selected files, folders, secrets, generated binaries, or sensitive project types." | MUST support context exclusion policy | Users may exclude files, folders, secrets, generated binaries, and sensitive project types |
...
| BS §9.1 | "Each managed process should have a process ID, working directory, start time, command, port, environment profile, output stream, and stop action." | MUST record all eight process attributes | No managed process exists without the complete attribute set |
| BS §9.1 | "Nirman should manage only processes that it started or that the user explicitly connected to the project." | MUST manage only owned or attached processes | A process is managed only if Nirman started it or the user attached it explicitly |
| BS §9.2 | "Nirman should detect the presence and versions of tools required by the selected project type" | MUST detect | Runtime MUST detect all listed tools |
| BS §9.2 | "The diagnostic screen should distinguish between installed, missing, outdated, misconfigured, and inaccessible tools." | MUST report all five tool states distinctly | A tool is never reported in an undifferentiated state |
| BS §9.3 | "should implement command timeouts" | MUST implement | Runtime MUST implement all listed controls |
| BS §9.3 | "Commands should be categorized as safe, reviewable, or privileged." | MUST classify every command into exactly one of the three | Classification precedes execution under the §9.3 command policy |
| BS §9.3 | "The interface should show multiple worker terminals separately, with searchable rolling logs and preserved raw artifacts for long-running processes." | MUST show per-worker terminals with searchable logs | Raw artifacts are preserved for long-running processes |
| BS §9.4 | "should be visible" | MUST be visible | Network access MUST be visible |
| BS §9.4 | "Dependency installation and API calls may require network access, but the user should see when it is being requested." | MUST surface network requests to the user | The request is visible when it is made |
| BS §9.4 | "Generated application runtime traffic should be distinguishable from Nirman’s own provider and package-manager traffic." | MUST separate generated-app traffic from Nirman traffic | Generated runtime traffic is attributable separately from provider and package-manager traffic |
| BS §10.1 | "By default, Nirman may read and write only inside the selected project workspace and its approved temporary build directories" | MUST stay inside workspace | Default: workspace-only access |
| BS §10.2 | "The command runner should validate the executable, arguments, working directory, and requested permissions" | MUST validate | Command runner MUST validate before execution |
| BS §10.2 | "It should reject suspicious path traversal, unexpected shell chaining, destructive commands, and commands that attempt to access protected locations unless the user explicitly approves them." | MUST reject the listed dangerous command classes | Path traversal, shell chaining, destructive commands, and protected-location access are rejected unless explicitly approved |
| BS §10.3 | "should show which dependencies" | MUST show | MUST show dependencies before install |
| BS §10.3 | "It should record package versions and update the project lockfile." | MUST record versions and update the lockfile | Every installation records versions and writes the project lockfile |
| BS §10.4 | "should detect likely secrets" | MUST detect | MUST detect and protect secrets |
| BS §10.4 | "It should warn before showing environment files in the editor and should never include secrets in generated explanations." | MUST warn before showing environment files and never expose secrets | A warning precedes display; secrets never appear in generated explanations |
| BS §10.5 | "should require explicit confirmation" | MUST require confirmation | Release/signing/publishing requires confirmation |
| BS §10.6 | "The activity log should record the task ID, user request, provider, model ID, files read, files changed, commands executed, network actions, approvals, test results, and artifact paths" | MUST record | Activity log MUST record all listed fields |
| BS §10.6 | "The log should be exportable for troubleshooting." | MUST make the log exportable | The exported log is sufficient for troubleshooting |
| BS §12.1 | "The user must be able to create a new Android project by describing it, open an existing local project, rename a project, close a project, inspect project health, and select the active AI provider" | MUST support creation | User MUST be able to create by describing |
| BS §12.2 | "The user must be able to describe a new application or request a change to an existing project" | MUST support description | User MUST be able to describe new app |
| BS §12.3 | "The user must be able to inspect generated files, review diffs, accept or reject grouped changes, manually edit files, create checkpoints, undo a task, and restore an earlier checkpoint" | MUST support inspection | User MUST be able to inspect files |
| BS §12.4 | "Nirman must start a local preview for supported Android projects" | MUST start preview | MUST start preview for supported projects |
...
| BS §26.4 | "Tie-breaking should prefer the change that satisfies the current acceptance criteria, preserves existing public behavior, passes more validation, and changes fewer unrelated files" | MUST apply the four criteria in the stated order | 1 satisfies acceptance criteria · 2 preserves existing public behaviour · 3 passes more validation · 4 changes fewer unrelated files. First discriminating criterion decides |
...
| BS §26.10 | "A visual test should launch the same flow across selected Nirman-managed emulator profiles, compare screenshots, and record profile-specific findings" | MUST do all three | Per selected profile. A profile that fails to launch is recorded as a failure, never skipped silently |
...
| BS §23.1 | "A project-context record should contain:" | MUST contain the declared context categories | The record carries exactly the context categories tabulated in §23.1 |
...
| BS §23.9 | "A normalized task result should contain:" | MUST contain the projected TaskResult fields | The record is the `TaskResult` projection fixed in nirman-schemas.md §1.11 |
| BS §23.9 | "The interface should render these events in real time and persist them in the activity log." | MUST render events live and persist them | Every rendered event is also persisted in the activity log |
...
| BS §23.12 | "Hooks should be deterministic where possible and should run outside the model’s control" | MUST run outside model control | No prompt, instruction, skill, or project file may disable a mandatory hook |
...
| BS §23.16 | "During execution, it should continuously report usage." | MUST report usage continuously during execution | Usage reporting is live for the duration of execution |
...
| BS §19 | "The second slice should add checkpoints, diffs, tests, repair attempts, Nirman-managed local Android emulator preview, and cancellation" | MUST add all six | Cancellation must actually interrupt running work, not merely mark intent |
...
| BS §20 | "Its differentiator should be the combination of:" | MUST hold the enumerated differentiators | The differentiator is the combination listed in §20, not any single item |
...
| BS §47.8 | "Nirman SHOULD measure worker and strategy quality using success rate, regression rate, time-to-evidence, false-positive review rate, repair reuse rate, handoff completeness, affected-test precision, and rollback frequency" | MUST measure all eight | success rate, regression rate, time-to-evidence, false-positive review rate, repair reuse rate, handoff completeness, affected-test precision, rollback frequency; metrics gate validated-pattern promotion per §47.8 |
...
| TA §23.3 | "An evidence record should contain" the tabled fields | MUST contain all twenty-eight `EvidenceRecord` fields | The twelve observation fields (`evidenceId` … `reproducibilityStatus`) plus the sixteen §5.7.4 identity and dependency fields (`sourceEventId` … `invalidationReason`); a record missing any field is rejected at capture rather than stored partially |
...
| TA §24.4 | "The model gateway should convert all supported protocols into a canonical internal request" | MUST convert every protocol into the `ModelRequest` shape | The eighteen `ModelRequest` fields and the seven `ReasoningSettings` fields are the sole request representation the agent runtime constructs; protocol-specific request objects exist only inside adapters |
| TA §24.4 | "The canonical response should be event-oriented" | MUST represent every response as an ordered `ModelEvent` sequence | The nine event types are the closed set; `sequence` is strictly increasing per `requestId`; a non-streaming provider still yields at minimum started, completed or failed, and usage events |
...
| TA §19 | "Large projects use sharded indexes, symbol-level summaries, dependency fingerprints, cache invalidation, and background compaction" | MUST apply all five once the repository map classifies the project as large | Below that scale the simple path is permitted; the classification point is an internal repository-map decision, so §80.3 gains no row and §80.1 rule 2 forbids inventing a default or an override range |
...
The unit of coverage is the §80.2 resolution row. One row resolves one "should" statement together with every verbatim recurrence of that statement in the same section, so the row count is lower than a raw count of the word "should". The figures below are the number of §80.2 rows whose Section column carries the scope prefix; the contract-graph verifier (§67.11) recomputes them from the §80.2 table on every run and reports a defect if this table disagrees with it. A row count alone proves only that every row is resolved, not that every statement has a row, so the verifier also checks the reverse direction: every "should" statement in the build spec outside §80, outside fenced examples, and outside table rows MUST be matched verbatim by the quoted statement of some §80.2 row. Coverage is therefore 100 percent in both directions — statements to rows and rows to statements — and a "should" added without a row is a certification failure, not a silent shortfall.
...
`ConversationDecision` is likewise a conversation-local proposal envelope. Only `ConstraintRegistry` may admit it as a canonical `LockedDecision`, return `canonicalDecisionId`, revise it, supersede it, invalidate it, or determine that it is locked. Conversation proposal status and canonical decision lifecycle are separate dimensions; a conversation cannot set authority by storing `locked = true`.
