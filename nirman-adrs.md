...
**Amended by ADR-227:** the taxonomy is nineteen roles — the fourteen above plus Emulator Driver Worker, Diagnostic Worker, Content Worker, Integration Double Worker, and Critic Worker. The one-taxonomy rule is unchanged; only the list grew, by closure over roles the documents already used.
...
The read-only roles form the default fan-out: the orchestrator may run a Repository Scout, a Requirements Planner, an Architecture Worker, a Security Worker, a Performance Worker, and a Critic Worker in parallel against the same revision without reservations, because none of them mutates; write-capable roles remain reservation-bound (build spec §54).
