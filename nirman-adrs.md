# Nirman Architecture Decision Records

This document holds every architecture decision record (ADR) for Nirman, in numeric order, exactly as accepted. The decision process — purpose, status values, the ADR format, and the review rules — lives in `nirman-decisions.md`; this document holds the decisions themselves (ADR-220). Accepted ADRs are the first rank of document precedence: they lock architectural and product decisions that `nirman-build-spec.md`, `nirman-technical-architecture.md`, and `nirman-milestones.md` then specify, implement, and sequence. A bare `§N` citation inside an ADR refers to the Build Spec unless the sentence qualifies it.
...
**Amended by ADR-222:** the production installation carries a third, internal executable, `NirmanWorker.exe`, which the supervisor spawns once per worker lease and which is never user-facing, never started by the user, and never a launcher; the one-product contract, the two long-lived processes, and the installer rule ("one installer installs all of them") survive unchanged.
...
**Amended by ADR-218:** “visible token budget” in this decision means the provider's context capacity — a technical representation constraint fitted by `ContextCapacityPlanner` (TA §59) — never an AI-usage budget, spend ceiling, or execution control; no such budget exists. The retrieval-within-capacity decision and the repository map it scopes survive unchanged.
...
**Placement (aligned with ADR-002A):** the controller is the bootstrap stage of `NirmanSupervisor.exe`, not a separate process; the replaceable half is the versioned application directory (`Nirman.exe` plus the supervisor's control-plane modules). See technical architecture §25.2 and §57.4.
...
**Consequences:** Both state machines require durable sequence numbers, impossible-transition checks, and replayable events. Under ADR-222 the two machines live in different processes: the loop state is the supervisor's ledger record and the process lifecycle state is that of a disposable `NirmanWorker.exe`, so a worker process can be terminated and relaunched without the loop losing a step.
...
**Amended by ADR-241:** registry membership is machine-checkable — a registered name owns a field block in `nirman-schemas.md` or is enumerated in §3.1's prose-defined identity list; every other statement of this decision survives unchanged.
...
**Consequences:** M0 delivers `tools/verify.sh` and `tools/verify.ps1` and requires them to remain aligned; until M0 lands, `tools/verify_contract_graph.py` and `tools/test_verify_contract_graph.py` are the complete available local gate and AGENTS.md must say so. Documentation, implementation, and fixture changes must be validated locally before commit. Optional hosted workflows, if ever reintroduced, must call or reproduce the local certification contract and must not be treated as the source of truth.
...
Physical Android hardware is outside Nirman's product scope and MUST NOT be used as a preview, validation, recovery, completion, fallback, or delivery dependency.
...
**Rationale:** The four canonical documents grew past the point where an agent can hold one of them in a single attentive read (the Build Spec alone exceeds 7,900 lines), and every earlier reduction proposal failed for the same reason: it moved text without saying who owned it afterwards. ADR-189 already made ownership a machine-readable property of a schema rather than of the prose around it, so a schema block can live in one place while its owner section keeps authority — provided the link is explicit in both directions and verified. Byte-identical ADR relocation and a whole-file milestone rename are the two moves that cannot lose meaning, so they are made unconditional; the schema move is the only step that touches many verifier rules, so it is the only step given a projection grammar the verifier can parse. Navigation and terminology material is useful precisely because it repeats what the owners say, which is why it is generated or checked and denied authority.

**Consequences:** `tools/verify_contract_graph.py` replaces its fixed four-document set with a document registry, resolves the new qualifiers, checks projection lines in both directions, enforces one fence per schema name across the root, regenerates `INDEX.md` and fails on drift, rejects authority markers in the reference documents, and reports the exact root document set; `tools/test_verify_contract_graph.py` copies every registry document into its fixtures and carries a negative case for each new rule. The migration lands as one gated commit per step, each leaving the repository certifiable: this decision and the AGENTS document set first, verifier machinery next, then ADRs, milestones, schemas in reviewable batches, and finally `INDEX.md`, `GLOSSARY.md`, and the wording sweep that retires "four canonical documents". Until the final step lands, AGENTS.md carries a transition note naming which documents already exist. Documentation relocation changes no capability maturity, certification, completion, or implementation status; every §5.6 capability reads exactly as it did before the move.
...
**Rationale:** Before this record the specification named the emulator as "Nirman-managed" without saying which engine it was or how it reached the machine; BS §4.2 still told the user to install missing tools from a guide, which is a hidden human dependency under BS §69.10 and leaves an agent at M9 with nothing to launch. Building an emulator is not an option: the engine is a hypervisor-backed QEMU fork plus an Android system image whose build is Linux-only and needs hundreds of gigabytes — both outside a Windows-only product that forbids WSL, VMs, and containers — and a self-built image has no Google Play services, so maps, push, billing, and sign-in would not run, contradicting BS §5. Bundling is not an option either: the SDK licence is granted to the user and is non-sublicensable, so the components must be fetched by the user's machine under the user's acceptance. Provisioning on first launch gives the user what "prebuilt" means in practice — an emulator that is configured, warm, and already rendering inside Nirman before the first project — while keeping Nirman's installer small, the components current, and the licence flow honest. Fixing the frame-path mechanics in the same record closes the second half of the same gap: TA §10.8 named `RenderTransport` without a schema, an owner process, a frame format, or a presentation surface.
...
**Rationale:** Before this record TA §3.3 said a worker "runs as a child process or isolated runtime task", an either/or that two agents would resolve differently, and nothing said whether a worker held the provider key, could touch its workspace, or could take the supervisor down with it. Hosting workers as tasks inside the supervisor is the simplest build, but one worker's panic, runaway allocation over a 200k-token context, or blocked thread then ends every task and the recovery scan itself, which contradicts the crash-isolation the supervisor exists to provide, and the restricted-token and Job-Object language the specification applies to workers means nothing for a task. A process per worker is the only host on which every isolation sentence already written — restricted token, Job Object, ACL-scoped workspace, process-tree supervision, resource quota — is literally true, and it is crash-only by construction: a worker that dies or hangs is killed through its Job Object and its lease re-issued, which the recovery ladder already assumes. Removing the provider key, the workspace, the network, and the ability to spawn from the worker process is what makes that process safe to treat as untrusted: prompt-injected model output can propose anything and obtain nothing except what `PolicyAuthority` and `ToolBroker` grant, and TA §3.3's phrase "may use the provider router" is satisfied by the router that lives in the supervisor. A hybrid — tasks for low-risk roles, processes for high-risk ones — was rejected because it doubles the code paths, tests, and failure modes of the same behaviour and lets the declared profile silently decide the execution model. The cost is one more executable to install and version and the serialization of every model call and proposal across a pipe; both are bounded and neither is on a per-frame or per-token hot path.

**Consequences:** TA §3 gains the process inventory, worker-host profile, placement rule, lifecycle, and edge table of §3.5, and §3.3 no longer offers a task as an alternative; TA §57.1 splits the former `nirman-agents` row into `nirman-worker-ipc`, `nirman-kernel`, and `nirman-agents` and states which crates each binary links; TA §57.2 draws the worker tier, §57.11 defines `WorkerConnection` and `WorkerConnection` becomes SCHEMAS §2.90; TA §14, §39, §44.2, §58.1, and §71.1 name the process boundary. BS §6.1.1 lists the third executable and invariant 12, §51.1 and §51.2 describe the worker tier, and §80.3 gains the worker process memory limit. M0 adds the third binary and the per-binary link check, M5 spawns its single worker as a process, M6 proves the worker host's four refusals, and M8's three workers are three processes. The verifier rejects a worker described as a task or thread, a worker that calls a provider or opens a file directly, a §57.1 table without the three-way split, a `WorkerConnection` without its owner-line block, and an ADR-222 without these terms. Nothing here changes what a worker may decide: it proposes, and the authorities of TA §21 decide, as before.

**Reversal trigger:** Measured evidence from the M5–M8 fixtures that per-lease process launch and pipe serialization make the worker cycle materially slower than the provider round trip it wraps and that the cost cannot be recovered by a pre-warmed process — in which case a superseding ADR may introduce a supervisor-owned pool of pre-launched `NirmanWorker.exe` processes, still one lease per process at a time, still under this record's token, Job Object, and no-authority rules; or Windows removes AppContainer or Job Object semantics on which the worker-host profile depends, in which case the superseding ADR names the replacement isolation primitive.
...
**Rationale:** Some forty names — `ToolBroker`, `TaskScheduler`, `CheckpointManager`, `LeaseManager`, `RequirementAuthority`, `ProjectMemoryStore`, `GoalInterpreter`, `TaskGraphCompiler`, and the continuity and export aliases among them — appeared only in process trees, module graphs, or a single sentence, so two agents implementing the same tree would place one name in different crates, give it different decision rights, or read an alias as a new authority. The naming-identity rules of the M19 audit covered four such pairs one literal at a time; the remaining names had no rule, and nothing stopped the next edit from adding another undefined name. A registry with a machine-checked closure rule turns "every authority is named once" from a review habit into a certification condition, and gives the M0 crate layout its component-to-crate map.

**Consequences:** TA §57.12 holds the registry and the mapping of the §21 hierarchy onto registered components; TA §57.1 gains two crates and the extended `nirman-control-plane`, `nirman-kernel`, `nirman-android`, `nirman-preview`, `nirman-evidence`, and `nirman-agents` rows; TA §21, TA §44.2, and build spec §77.4 point at the registry; the `EvidenceStore` label in TA §71.1 becomes the evidence ledger of `EvidenceAuthority` and the malformed TA §65.1 row is repaired; AGENTS.md §3 and GLOSSARY.md name the registry; M0 gains the registry-parity check; the verifier enforces the closure rule, the row shape, the crate agreement with §57.1, and the alias-target rule, with harness negatives for a dropped row, an undefined name, a blank crate cell, a crate that contradicts §57.1, and an alias that claims a crate.
...
**Rationale:** The policy side of autonomy is already complete: build spec §23.7 and §29.7 forbid routine prompts, §28.2 forbids blind retries, §69.11 forbids mid-run questions, §69.10 forbids silent running, and the frozen battery scores any human touch as failure. What still turns a prompt into an attended session is the perception → action → repair side of the emulator loop: a blind agent, no scenario author, dialogs classified as human work, no deterministic baseline, a question with nobody to answer it, a backend that does not exist yet, repairs that never get cheaper, workers colliding on Gradle and manifest files, and one emulator serving two masters. Each mechanism removes one of those and none adds authority to the model: the runtime, not the model, performs every one of them.

**Consequences:** Build spec §29.3, §54.2, §56.2, §69.10, §69.11, and §76.5 and technical architecture §10.2, §10.3, §10.4, §51.1, §60.1, §62.1, §73.12, §74.1, and §74.2 gain the mechanisms; `nirman-schemas.md` gains `ScreenModel`, `ScreenGraph`, `DeviceHygienePolicy`, `GoldenSnapshot`, `ClarificationRecord`, `ContractDouble`, `RepairPattern`, and `SharedSurfaceChangeRequest`; TA §57.12 gains the registry rows; milestones M8, M20, M26, M31, and M82 gain work items. No new milestone, executable, crate, or capability is created; ADR-222's process model and ADR-224's scope boundary are unchanged — every mechanism runs inside the supervisor or a worker, on the local emulator, before the certified artifact exists. Evidence honesty is preserved by construction: double-backed evidence is labeled `DOUBLE_BACKED`, assumption-backed requirements are labeled `ASSUMED`, and a vision model's absence marks visual criteria `NOT_OBSERVED` instead of blocking functional completion.
...
**Consequences:** Build spec §23.3 becomes the single-mode statement; §23.7's example policy loses its mode column and its pause sentence; §26.15, §27.1, §27.5, §28.5, §29.7, §52.2, §66.10, §69.10, §69.11, §72, and §79.11 drop attended/unattended conditionals; technical architecture §4.3, §7.5, §11.4, §16.2, §16.2.1, §16.2.2, §34.3, §34.5, and §57.4 follow; build spec §4.3 and the §80.3 liveness defaults are adjusted; `AgentProfile.autonomy_level` and `GoalContract.autonomyPolicy` are removed; `LoopHeartbeat` is added to nirman-schemas.md; the §80.2 rows quoting the withdrawn sentences are replaced; M21 and M31 are retitled; ADR-048 is superseded and ADR-171 amended in place. No new executable, authority, or budget is introduced.

**Amendment (rule 4 closure):** rule 4 was stated but three divergent stop lists survived it — §27.7 listed six conditions, §27.10 listed nine, and §27.11 listed seven, and the §80.2 row resolving the §27.7 sentence read "MUST stop only on those four conditions". §27.10 is amended in place to become the single owner of the stop vocabulary, split into requirement-level `BLOCKED`/`USER_REQUIRED` decisions (which the goal continues around) and a closed set of five goal-level terminal conditions; §27.7 and §27.11 now cite it rather than restating a list, and `PARTIALLY_BLOCKED` is confined to the §27.10 completion-classification vocabulary so it cannot become a fourth state set. Three §80.8 prompt templates that instructed the model to "Stop and escalate if blocked", "Stop and report if a check fails", and "Escalate if the cause is unclear" are rewritten to record a requirement-level decision and continue independent work. `RecoveryAttempt.continuationDecision` gains closed preconditions per value, and `TERMINATE` is legal only when it records one of the five goal-level terminal conditions. Technical architecture §28.1 is declared the canonical recovery ladder and §80.4.1 now carries its levels verbatim with applicability preconditions instead of a second, differently ordered list, so "recovery level 8 or 9" has one meaning. The contract-graph verifier gains three semantic-documentation rules — stop-condition closure, prompt stop language, and recovery ladder ownership — each with proving mutations in `tools/test_verify_contract_graph.py`. No new executable, authority, budget, state, or schema field is introduced beyond typing the existing `AgentLoopRecord.state` with technical architecture §71.4's own names.
...
## ADR-227: Complete agent taxonomy — twenty-one canonical roles, three-level delegation, and the read-only critic pair

**Status:** Accepted
**Locks:** `CONTRACT.RUNTIME.AUTHORITY`

**Decision:** The canonical worker taxonomy of ADR-049 grows from fourteen roles to twenty-one, and the growth is closure, not invention: every role added is one the build spec or the architecture already *uses* without a registry row. The seven added roles are (1) **Emulator Driver Worker** — the "emulator worker" of technical architecture §10.2: installs, launches, drives scenarios and `ScreenGraph` exploration through `AndroidDeviceAdapter`, applies `DeviceHygienePolicy`, and returns structured runtime evidence; (2) **Diagnostic Worker** — the "narrowly scoped diagnostic child" every worker may spawn: read-only root-cause localization (§63.2) producing a `FailureContextPackage`; (3) **Content Worker** — the `ContentWorker` of §85.2: product copy, localization, and accessibility text as `ContentMutation` proposals; (4) **Integration Double Worker** — authors and conforms `ContractDouble` fixtures (§74.1) so integration-heavy applications keep executing offline scenarios; (5) **Critic Worker** — the read-only adversarial reviewer that hosts `StrategyCritic` (§72.7) and the independent pre-promotion review of build spec §47.3; it emits findings and evidence requests only; (6) **Android Platform Worker** — owns Android SDK/platform APIs, Kotlin/Java interop, JNI/native modules, Gradle/plugin integration, lifecycle/process/background execution, permissions, services, device APIs (BLE/NFC/camera/sensors), widgets, OS-version compatibility, and platform-specific diagnostics; (7) **Backend & Service Engineering Worker** — owns the supporting backend/service layer the Android app consumes: REST/GraphQL API implementation, server-side business logic, database/server schema, authentication/authorization backend, webhooks, server-side validation, background jobs/queues, cloud functions/serverless, API versioning, backend integration tests, and deployment configuration for the user's external backend. The three role tables (build spec §22.1, §23.4; technical architecture §6.5) list exactly these twenty-one names, and a worker phrase in either document that is not one of them is a documentation defect.
...
## ADR-230: One cycle machine at two resolutions — TA §71.4 is canonical, BS §52.2 is its projection

**Status:** Accepted · **Supersedes:** none · **Amended by:** none

**Decision:** The kernel reasoning cycle has exactly one state machine, and it is the thirteen-state machine of technical architecture §71.4. That section is the single authority over cycle transitions. Build spec §52.2 is withdrawn as a second state machine and reconstituted as the **coarse durable-projection vocabulary**: the nine names under which cycle states are recorded in `LoopHeartbeat.stateEntered` and reported to the supervisor. The relationship between them is a total, surjective projection, fixed in build spec §52.2 and reproduced here: `OBSERVE`←`OBSERVE`; `UNDERSTAND`←`UNDERSTAND`,`HYPOTHESIZE`; `PLAN`←`STRATEGIZE`,`SPECULATE`; `SELECT_ACTION`←`SELECT`; `AUTHORIZE`←`AUTHORIZE`; `EXECUTE`←`EXECUTE`; `OBSERVE_RESULT`←`OBSERVE_RESULT`,`REFLECT`; `UPDATE_STATE`←`UPDATE`; `EVALUATE_PROGRESS`←`DECIDE`,`DELEGATE`. Every fine state maps to exactly one coarse state and every coarse state has at least one fine preimage.

The former `VALIDATE` branch of `EVALUATE_PROGRESS` is folded into `DECIDE` and withdrawn as a cycle branch. Validation is owned by the completion evaluator and the evidence gates; a validated goal leaves the cycle through `DECIDE terminate -> COMPLETED`, never through a separate validation transition. The remaining coarse branches are the coarse view of the §71.4 `DECIDE` edges, and the §71.4 `branch` edge projects onto `PLAN` through `SPECULATE`.

**Rationale:** Two closed, mandatory state machines described the same object, with five names shared out of nine and thirteen, and the two durable records that observe the cycle bound to opposite sides — `LoopHeartbeat.stateEntered` to §52.2 and `AgentLoopRecord.state` to §71.4. Nothing mapped between them, so an implementing agent had to choose a machine or invent a mapping. An isomorphism was impossible: `HYPOTHESIZE`, `STRATEGIZE`, `REFLECT`, and `SPECULATE` have no §52.2 counterpart, and §52.2's branch set had no branch or speculate path at all, while §66.6 requires discriminating tests for open hypotheses, §80.4.1 makes strategy selection normative, and §88 provides speculative branching. A projection rather than an isomorphism resolves this without discarding either surface: the fine machine keeps full expressive power and remains authoritative for transitions, while the coarse vocabulary stays small enough for the supervisor's liveness scan and keeps `LoopHeartbeat` unchanged. Technical architecture owns the machine because ADR-220 assigns implementation architecture, protocols, and module boundaries to the Technical Architecture; a cycle state machine is implementation architecture. This follows the same discipline build spec §33.2 already applies to `TaskExecutionState`, `ProductLifecycleState`, kernel cycle outcomes, and completion classifications.

**Consequences:** Technical architecture §71.4 declares itself the canonical cycle machine. Build spec §52.2 restates its nine names as a projection vocabulary, carries the projection table, and loses `VALIDATE`. `LoopHeartbeat.stateEntered` keeps its nine values and gains a statement that they are the build spec §52.2 projection; `AgentLoopRecord.state` keeps the thirteen technical architecture §71.4 values and gains the reciprocal statement. Build spec §29.4 now attributes heartbeats to technical architecture §71.4 transitions carrying the coarse projection, rather than to §52.2 transitions. The six terminal cycle outcomes are unchanged and remain neither fine nor coarse cycle states. The contract-graph verifier gains a cycle-projection closure rule with proving mutations. No new state, schema field, authority, contract, clause, milestone, or budget is introduced, and no existing state is renamed.

**Reversal trigger:** Runtime evidence that the coarse projection loses information the supervisor needs — for example a hung loop whose last coarse state cannot distinguish two materially different fine states for recovery purposes. The repair is to refine the projection or the heartbeat, never to reintroduce a second transition authority.
...
**Decision:** A completeness claim of the form "everything of kind X is defined here" MUST state the scope it covers, and MUST be backed by a verifier rule that recomputes the claim rather than trusting it. Two such claims are withdrawn and restated. Build spec §80.8 no longer says that every system prompt used by the runtime is defined there; it says that every template the section provides is defined there, and it enumerates the prompt classes the runtime is required to hold — coordinator, worker, skill, deliberation, and review, from build spec §69.2 and technical architecture §73.1, plus the fixed release-evaluation prompt set of development plan §16.3. The coordinator prompt is fully derived (§80.8.1); the worker, skill, deliberation, and review prompts remain owner-pending. Build spec §80.6 no longer says "every test fixture referenced in the specification"; it says "every test fixture referenced in the build spec", and attributes `FIX-DEL-01`–`FIX-DEL-07` to milestones §M95, where they are already normatively defined.

Nirman uses a chat-first request model. The user describes the Android application in natural language; the runtime normalizes that request into requirements and constructs internal model instructions as implementation artifacts composed from role instructions, the `IntentSynthesisPromptContract`, policy constraints, retrieved context, task state, evidence, and provider/model requirements. Nirman has no user-facing prompt-template, prompt-library, prompt-authoring, or prompt-selection product layer, and the original user request is preserved as task provenance (`AgentTask.userRequest`) rather than treated as a prompt-template object or as the assembled provider instruction. The eight `FIX-PROG-*` prompts are frozen evaluation fixtures, not user-facing templates, and are separate from the runtime's internal model-instruction surface.
...
## ADR-241: The canonical schema registry is closed over field blocks and declared prose-defined identities
...
1. Witness-set commit boundary. A `ConstructionTransaction` may enter the committed state only when its committed project-state witness set W = {workspace file tree, toolchain lock, dependency snapshot} differs from its base witness set; `W_after == W_base` forbids commit. The criterion is a delta in W, never a delta in workspace files alone: a transaction that changes only the toolchain lock or the dependency snapshot has a nonzero W-delta and is equally eligible to commit and mint. A nonzero W-delta is necessary for commit but is not sufficient: the policy, validation, mutation-safety, test, build, preview, and evidence gates of TA §45.3 are evaluated before the atomic boundary, and a transaction that fails any of them rolls back rather than committing.
