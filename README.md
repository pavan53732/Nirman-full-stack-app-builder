...
The repository is transitioning from the specification and documentation-certification stage into vertical implementation. M0/M1/M2 foundations are validated, and the current M115 slice connects the durable SQLite control plane to an authenticated Tauri command/event bridge and a React projection reducer. The M115 command registry and protocol schemas are implemented at the local control-plane boundary; full underlying use-case execution, supervisor connection, native Tauri runtime execution, React DOM evidence, worker orchestration, Android project synthesis, emulator/device integration, runtime certification, signing, and the Windows installer remain outstanding.
...
| Windows Nirman desktop implementation | Tauri host command/event bridge is implemented and Windows-target type-checked; native Windows runtime and installer validation remain outstanding |
| Rust/Tokio local control plane | Durable typed control-plane core connected to the desktop bridge; broader supervisor integration remains outstanding |
| React/TypeScript desktop UI | React consumes the typed host subscription/bootstrap/replay path; headless reducer evidence passes, while real Tauri event delivery and DOM/runtime validation remain outstanding |
...
Documentation certification must not be confused with runtime certification. Passing the verifier proves documentation structure, contract identity, graph reachability, semantic anchors, and mutation coverage. It does not prove that Nirman can yet synthesize an Android app, launch an emulator, produce an APK, recover after a reboot, or install a Windows executable. The current Rust and frontend checks validate only the implementation foundation and do not substitute for runtime certification. The M2 foundation trace proves file-backed ledger persistence, event replay, pause/resume/cancellation transitions, checkpoint reload, supervisor reconciliation, lease-fence replacement, and stale/out-of-order projection rejection. The M115 boundary trace proves file-backed command/projection persistence, persisted idempotency after restart, typed envelope round-tripping, and headless projection rejection semantics. It does not prove registered Tauri command invocation, Tauri event delivery, production React DOM receipt, Android runtime behavior, APK export, supervisor restart behavior, or full autonomous worker execution.
...
A browser-loaded Vite shell intentionally reports the host as unavailable; it does not use a mock or browser-local authority. The host uses Tauri 2.11.5 with the 2.11 JavaScript API line and requires Rust 1.77.2 or newer. The host event is `nirman://control-event`. The current implementation includes authenticated handshake, projection snapshot, subscription bootstrap, cursor replay, acknowledgement, heartbeat, close, and typed dispatch for the 13 canonical M115 commands plus compatibility reconnect/pause operations. Durable command-result records preserve idempotency across restart and conflicting request fingerprints are rejected. The machine-readable trace is `tests/evidence/desktop_ipc_trace.json`; it is explicitly a headless durable-boundary trace and does not claim registered Tauri command runtime, React DOM runtime, Android runtime, or APK export.
