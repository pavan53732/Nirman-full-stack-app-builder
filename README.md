...
3. **Specification-only work & AI blueprint.** All 28 registered capabilities are `PLANNED`. The active phase is strictly documentation and architectural perfection. These canonical specifications are authored specifically to serve as complete, unambiguous blueprints for AI agents to implement Nirman end-to-end without guesswork or hallucinations. Do not report any capability as `SUPPORTED`, `IMPLEMENTED`, or `verified` without its full contract chain, validation path, recovery behavior, and fixture evidence. Documentation certification (the verifier in `tools/`) proves document structure and graph consistency only — not runtime behavior.
...
**What makes this different from generating code:** every claim above is backed by an observation. "It builds" means Gradle exited zero. "It runs" means the generated Android application launched in Nirman's managed local Android emulator. "It is live in preview" means that runtime's actual rendering surface is projected into Nirman's Preview panel. "It works" means an executable interaction scenario produced the expected observed runtime state and assertion result, and the screenshot proving it is retained. For non-trivial requirements, "works" additionally requires state/failure/regression evidence — not merely a happy-path scenario. Nirman cannot report success it did not observe — that rule is a sealed clause in the build spec, not a convention.
...
| All 28 registered capabilities | `PLANNED` (BS §5.7 registry; §5.6 is the coverage matrix) |
...
- **Attention reliability, placement, and recall verification** (ADR-219): Nirman does not implement attention; providers do, and hybrid sparse/linear models recall distant literal content unevenly. Each provider model carries a measured `AttentionReliabilityProfile` (probed at profile save, learned from premise mismatches, `UNPROFILED` until measured). Constraints, locked decisions, and exact edit targets are placed in a DENSE block adjacent to the instruction and bounded by the measured reliable recall span; the sufficiency gate checks attendability as a seventh dimension; the mutation broker rejects proposals whose anchors or premises disagree with their context package as `PREMISE_MISMATCH` before any transaction; and compaction never carries constraints — they are re-projected and probe-verified. Probe results are telemetry only and never pause or throttle work.
...
| [`nirman-schemas.md`](nirman-schemas.md) | Every fenced field-list schema and the `CanonicalSchemaRegistry` list, each block under an owner line naming its Build Spec or Technical Architecture section |
