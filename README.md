...
| Platform capability contract (ADR-206, M118) | Deterministic core implemented in `nirman-domain`/`nirman-tools`: `TargetPlatformResolver`, canonical capability matrix, observation-driven classification, cross-build admission and native-validation gates, `WorkerContract` platform fields; TEST-PLAT-001 fixtures A–D plus the §84.5 addenda pass headless (EV-PLAT-001); real OS preflight collection and any Windows runtime observation remain environment-dependent |
