...
The generated target is native Android and only native Android. The resolver may select Java, Kotlin, Android Views, Jetpack Compose, Android native modules using NDK/CMake, Gradle plugins, Android device APIs, Android background services, or a mixed native Android architecture. Cross-platform application frameworks (such as Flutter, Cordova, Ionic, web wrappers, and JavaScript hybrid runtimes) and retired cross-platform stacks are outside current scope under ADR-257.
