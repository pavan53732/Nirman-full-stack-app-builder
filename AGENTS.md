...
Every agent must follow the canonical Markdown documents of §1 exactly — the Build Spec, the Technical Architecture, the schema document, the milestone document, and the ADR document. They are binding engineering contracts, not background reading or optional recommendations. Before planning, coding, editing documentation, installing dependencies, changing configuration, or running a potentially destructive command, the agent must read the relevant sections and inspect the current repository state. For a cross-cutting change, the agent must inspect all five canonical documents of §1 and the related registry, verifier, and milestone entries.
